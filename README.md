...
| `horizontalGesture`     | `boolean` | `false`                | ![Smooth only][smooth-only]<br>If you want to use horizontal input to scroll in your page                                                                                                                                                                                                          |
...
| `resetNativeScroll` | `boolean` | `true`                | Sets `history.scrollRestoration = 'manual'` and calls `window.scrollTo(0, 0)` on locomotive-scroll init. Useful if you use transitions with native scrolling, otherwise we advise to set it to `false` if you don't want to break History API's scroll restoration feature. |
